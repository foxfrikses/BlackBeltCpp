...
void TestSaveLoad() {
  ...
  */
  ...
  stringstream mySs;
  std::vector<std::map<std::string, std::string>> myMap1{
      {
          {"first", "first"},
          {"second", "second_value"}
      },
  ...
  std::vector<std::map<std::string, std::string>> myMap2;
  Serialize(myMap1, mySs);
  Deserialize(mySs, myMap2);
  //ASSERT_EQUAL(myMap1, myMap2);
}
